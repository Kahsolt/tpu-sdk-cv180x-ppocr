...
int main(int argc, char *argv[]) {
  if (argc != 4) {
    printf("Usage: %s chocr_rec.cvimodel image.jpg chocr_keys.txt\n", argv[0]);
    exit(-1);
  }
  ...
  printf("  output->shape: %d (%d, %d, %d, %d)\n", output->shape.dim_size, output->shape.dim[0], output->shape.dim[1], output->shape.dim[2], output->shape.dim[3]); // 4 (80, 1, 5531, 1)
  ...
  // post-process: argmax()
  ts_start = clock();
  const size_t L = output->shape.dim[0], D = output->shape.dim[2];
  int last_idx = -1;
  printf("------\n");
  last_idx = -1;
  for (int k = 0 ; k < L; k++) {
